...
export class SetsService {
    ...
    private async allocateMembers(session: Session, createdSets: Set[]): Promise<void> {
    ...
    async viewSetForSession(sessionId: string) {
        return this.setRepository.findAndPopulate(
            { session: sessionId },
            ['players']
        );
    }
    ...
    async viewSingleSet(setId: string) {
        const set = await this.setRepository.findAndPopulate(
            { _id: setId },
            ['players']
        );
